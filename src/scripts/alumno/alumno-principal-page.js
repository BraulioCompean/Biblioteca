...
const modalBookImg = document.getElementById("modal-img");
const modalBookTitle = document.getElementById("book-title");
const modalBookAuthor = document.getElementById("book-author");
const modalBookEditorial = document.getElementById("book-editorial");
const modalBookCategory = document.getElementById("book-category");
const modalBookSynopsis = document.getElementById("book-synopsis");

const infoBooksCache = {};
...
bookCards.forEach((element) => {
    //AGREGAMOS UN EVENT LISTENER A CADA ELEMENTO QUE TENGA LA CLASE book-card
    element.addEventListener("click", (event) => {
        const isbn = event.target.closest(".book-card").id;

        if (infoBooksCache[isbn]) {
            updateModal(infoBooksCache[isbn]);
        } else {
            fetch(`../../db/info-libro.php?isbn=${isbn}`)
                .then((response) => response.json())
                .then((data) => {
                    infoBooksCache[isbn] = data;
                    updateModal(data);
                });
        }
    });
...
function updateModal(data) {
    modalBookImg.setAttribute("src", data.imagen);
    modalBookTitle.innerText = data.titulo;
    modalBookAuthor.innerText = data.autor;
    modalBookEditorial.innerText = data.editorial;
    modalBookCategory.innerText = data.categoria;
    modalBookSynopsis.innerText = data.sinopsis;
...
addFavoritesBtn.addEventListener("click", () => {
    if (addFavoritesBtn.textContent.trim() == "Agregar a Favoritos") {
        addFavoritesBtn.innerHTML =
            favoritesSvgPressed + "Agregado a Favoritos";
